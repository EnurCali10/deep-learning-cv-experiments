"""
Problem:video üzerinden yolo ile arac sayimi
-car
-otobüs
-truck

Yolo ile tracking sonraasinda belirli bir cizgiyi gecen araclarin sayisini tutmak istiyorum.
data:https://www.kaggle.com/datasets/benjaminguerrieri/car-detection-videos
"""
...
def get_line_side(x,y,line_start,line_end):#obje line in hangi tarfinda anlamak icin kullanilir
...
line_start = (int(frame_height*0.2), frame_height)
...
while True:
    ...
    if not success:
        exit('Video acilmadi')
    frame = cv2.resize(frame, (0,0), fx=1.0, fy=1.0)#boyut ayarlama
    #tracking arac takibi
    results= model.track(frame, persist=True, stream=False, conf=0.5, iou=0.5, tracker="bytetrack.yaml")

    if results[0].boxes.id is not None: #eger takip edien obje varsa
        ids= results[0].boxes.id.int().tolist()#tüm id leri almis olduk
        classes = results[0].boxes.cls.int().tolist()#tum classeslari aldik
        ...
        for i, box in enumerate(xyxy):
            ...
            previous_side = object_last_side.get(track_id, None )# bir önceki frame de hangi atarfta oldugu 
            object_last_side[track_id] = current_side#last side güncelleme
